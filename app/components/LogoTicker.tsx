...
export default function LogoTicker() {
  return (
    <section className="relative py-20 border-y border-border overflow-hidden">
      <Reveal>
        <p className="text-[11px] uppercase tracking-[0.15em] text-muted text-center mb-10 font-medium">
      ...
      <div className="flex overflow-hidden">
        <div className="flex items-center gap-20 logo-ticker">
          {[...logos, ...logos, ...logos, ...logos].map((logo, i) => (
            <div
              key={`${logo.name}-${i}`}
              className="flex-shrink-0 opacity-50 hover:opacity-80 transition-opacity duration-500"
            >
              <Image
                src={logo.src}
                alt={logo.name}
